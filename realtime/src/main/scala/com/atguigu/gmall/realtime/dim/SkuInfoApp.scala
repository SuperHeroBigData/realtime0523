...
import com.atguigu.gmall.realtime.utils.{HbaseUtils, KafkaStreamUtils, OffsetManagerUtil}
...
/*
@author zilong-pan
@creat 2020-10-30 13:02
@desc $  
*/
object SkuInfoApp {
  def main(args: Array[String]): Unit = {

    val sparkConf: SparkConf = new SparkConf().setMaster("yarn").setAppName("SkuInfoApp")
    ...
    val skuInfoDstream: DStream[SkuInfo] = objectDstream.transform {
      rdd =>{
        if(rdd.count()>0){
          ...
          val tmList: List[JSONObject] = HbaseUtils.query(tmSql)
          ...
          val category3List: List[JSONObject] = HbaseUtils.query(category3Sql)
          ...
          val spuList: List[JSONObject] = HbaseUtils.query(spuSql)
